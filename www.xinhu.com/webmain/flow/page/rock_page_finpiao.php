...
$(document).ready(function(){
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'发票管理('+nowtabs.name+')',
				cont:'发票管理('+nowtabs.name+')的列表的',
				explain:'订阅[发票管理]的列表',
				objtable:a
			});
		},
		getacturl:function(act){
			return js.getajaxurl(act,'mode_finpiao|input','flow',{'modeid':modeid});
		},
		...
		daoru:function(){
			window.managelistfinpiao = a;
			addtabs({num:'daorufinpiao',url:'flow,input,daoru,modenum=finpiao',icons:'plus',name:'导入发票管理'});
		},
	...
	//表格参数设定
	var bootparams = {
		fanye:true,modenum:modenum,modename:modename,statuschange:false,tablename:jm.base64decode('ZmlucGlhbw::'),
		url:c.storeurl(),storeafteraction:'storeaftershow',storebeforeaction:'storebeforeshow',
		params:{atype:atype},
		columns:[{text:"发票类型",dataIndex:"ptype",sortable:true},{text:"开票日期",dataIndex:"opendt",sortable:true},{text:"销售方名称",dataIndex:"custname"},{text:"开票人",dataIndex:"kainame"},{text:"购买方名称",dataIndex:"maicustname"},{text:"金额",dataIndex:"money",sortable:true},{text:"发票代码",dataIndex:"daima"},{text:"发票号码",dataIndex:"haoma"},{text:"说明",dataIndex:"explain"},{
...
//[自定义区域end]
	...
	js.initbtn(c);
	var a = $('#viewfinpiao_{rand}').bootstable(bootparams);
	c.init();
