...
$(document).ready(function(){
	...
	var c = {
		...
		subscribelist:function(){
			js.subscribe({
				title:'订阅报表('+nowtabs.name+')',
				cont:'订阅报表('+nowtabs.name+')的列表的',
				explain:'订阅[订阅报表]的列表',
				objtable:a
			});
		},
		getacturl:function(act){
			return js.getajaxurl(act,'mode_subscribeinfo|input','flow',{'modeid':modeid});
		},
		...
		daoru:function(){
			window.managelistsubscribeinfo = a;
			addtabs({num:'daorusubscribeinfo',url:'flow,input,daoru,modenum=subscribeinfo',icons:'plus',name:'导入订阅报表'});
		},
	...
	//表格参数设定
	var bootparams = {
		fanye:true,modenum:modenum,modename:modename,statuschange:false,tablename:jm.base64decode('c3Vic2NyaWJlaW5mbw::'),
		url:c.storeurl(),storeafteraction:'storeaftershow',storebeforeaction:'storebeforeshow',
		params:{atype:atype},
		columns:[{text:"订阅标题",dataIndex:"title"},{text:"订阅内容",dataIndex:"cont"},{text:"添加时候",dataIndex:"optdt",sortable:true},{text:"订阅提醒对象",dataIndex:"recename"},{text:"文件",dataIndex:"filepath"},{
...
//[自定义区域end]
	...
	js.initbtn(c);
	var a = $('#viewsubscribeinfo_{rand}').bootstable(bootparams);
	c.init();
