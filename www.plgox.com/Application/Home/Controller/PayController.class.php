            ...
            if($data['trade_status'] == 'TRADE_FINISHED' || $data['trade_status'] == 'TRADE_SUCCESS') {
               if( payOrderLook($out_trade_no) ){
                    ...
                    if( $save_data ){
                        ...
                        foreach ( $Order_detil as $key => $value ) {
                            // 购买之后的库存进行减量
                            $stock = D("Cart")->where(array("cart_id"=>$value['order_cart_id']))->select();
    ...
    public function return_url2() {
       ...
       //计算得出通知验证结果
        ...
        if($verify_result) {//验证成功
            ...
            // 验证成功后执行的数据
            if($data['trade_status'] == 'TRADE_FINISHED' || $data['trade_status'] == 'TRADE_SUCCESS') {          
               if( orderChecked_($out_trade_no) ){
                    // 更改数据库订单状态
                    $save_data = order_xyj_save($out_trade_no,$buyer_email,$order_buyer_id,$trade_no);
                    if( $save_data ){
                      $this->redirect("User/my_dingdan");
                    }
               }
