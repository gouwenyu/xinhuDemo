<?php if (!defined('THINK_PATH')) exit(); echo W('Common/header_esxz');?>
<?php echo W('Common/header1_esxz');?>
<link rel="stylesheet" type="text/css" href="/Public/Home/css/help_top.css">
<link rel="stylesheet" type="text/css" href="/Public/Home/css/juezhi_fb.css">
<link rel="stylesheet" type="text/css" href="/Public/Home/css/fen_ye.css" />
 <style>
  tr{
  	line-height:143px;
  }	
  .jzfb2{
	padding-bottom: 0px;
}
.jzfb_bj_sc{
	width:77px;
}
.jzfb22{
	line-height:22px;
	text-align:left;
	margin-left:20px;
	margin-top:25px;

}
.current{
		background-color:#E70052 !important; 
		border:1px solid #E70052 !important;
		color:#FFFFFF;padding:8px 12px 8px 12px;
	}		
</style> 
<div class="mypj juezhi">

     <div class="mypj1_box">

	   	<div class="my_t2" >
	         <ul class="my_t5">
	           <li><a href="#1" class="help_topjt help_topjt_dj">发布管理</a></li>
	           <li><a href="<?php echo U('JueZhi/JueZhi_fbgl');?>" class="help_zl">发布中</a></li>
	           <li><a href="<?php echo U('JueZhi/JueZhi_shz');?>" class="help_zl">审核中</a></li>
	           <li><a href="<?php echo U('JueZhi/JueZhi_wtg');?>" class="help_zl">未通过</a></li>
	           <li><a href="<?php echo U('JueZhi/JueZhi_wdsc');?>" class="help_sh">我的收藏</a></li>
	           <li>
		  	     <a  href="#" class="hdmk">
		  	     </a>
			  </li>
	         </ul>
	      </div>

   	      
   	      
   	      <div  class="mypj2 jzfb">
             	
               	
              <div class="jzfb2">
              	   <div class="jz_wdsc">
              	   	 <a href="<?php echo U('JueZhi/JueZhi_wdsc',array('type'=>0));?>"><span class="jz_wdsc1">默认排序</span></a>
              	   	 <a href="<?php echo U('JueZhi/JueZhi_wdsc',array('type'=>1));?>"><span class="jz_wdsc1">价格低到高</span></a>
              	   	 <a href="<?php echo U('JueZhi/JueZhi_wdsc',array('type'=>2));?>"><span class="jz_wdsc1">价格高到低</span></a>
              	   </div>
	               <table class="myxy32">
	               	<tr>
	               		<td></td>
	               	</tr>
	               	
	               	<?php if(is_array($goods)): $i = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods_list): $mod = ($i % 2 );++$i;?><tr>
		    			<td class="wdsy_first">
		    				<input type="checkbox" class="choose_all"   value="<?php echo ($goods_list["collect_id"]); ?>"/>
		    			</td>
		    			<td style="width:458px;">
		    			  <div class="jzfbbox">
			    			  <div class="jzfb21" >
			    			   <img src="/Uploads/admin/<?php echo ($goods_list['goods_first_pic']); ?>"  name="jzfb21"/>
			    			  </div>
			    			  <div class="jzfb22" >
			    			    <div class="wdsc6">
			    			    	<b class="wdsc_dbj"><?php echo ($goods_list["goods_name"]); ?></b>
			    			    	<?php if($goods_list["goods_is_business"] == 1): ?><span class="wdsc_qy">企</span><?php endif; ?>
			    			    	
			    			    </div>
			    			    <div class="wdsc61">
			    			    	 	<?php echo ($goods_list["goods_name"]); ?>
			    			    </div>
			    			    <div class="wdsc62">
			    			    	 <?php echo ($goods_list["goods_province"]); ?>&nbsp;&nbsp;<?php echo ($goods_list["	goods_city"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($goods_list["goods_publish_time"]); ?>
			    			    </div>
			    			  </div>
			    		  </div>
		    			</td>
		    			<td>
		    				<span class="wdsc_ys"><?php echo ($goods_list["goods_deal_price"]); ?></span>
		    			</td>
		    			<td>
		    				<span class="jzfb_bj_sc" onclick="cancel_collect(<?php echo ($goods_list["collect_id"]); ?>)" >取消关注</span>
		    			</td>
		    			<input type="hidden" class="check_id" value="<?php echo ($goods_list["goods_id"]); ?>">
		    		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		    	
		    	</table>
             </div>
             
             <input type="hidden" id="all_check_id" value="">
             <div class="jzfb3">
             	<div class="jzfb31">
         		  <div class="jzfb21 jzfb310" >
	    			 <input type="checkbox" class="qx_button"  name="jzfb21"/>
	    		   </div>
	    	      <div class="jzfb2_xm" >
	    			  <span>全选</span>
	    	      </div>
             	</div>
             	
             	<div class="jzfb31">
         		  <div class="jzfb21 jzfb310" >
	    			 <input type="checkbox" class="sc_button"  name="jzfb21"/>
	    		   </div>
	    	      <div class="jzfb2_xm" >
	    			  <span>取消收藏已选项</span>
	    	      </div>
             	</div>
             </div>
             
             <div class="message" style="height:120px;padding:50px 0px;">
             	<?php echo ($page); ?>
             	</div>
             
             
             
          </div>
   	
   	
   	
 
   	   
   </div>
 </div>
 <input type="hidden" id="change_color" value="<?php echo ($change_color); ?>" />
 <!--遮罩-->
<div class="zhezhao" style="display:none;">

	<div class="zz_nr_scdz" style="display:none;">
	  <div class="zz_nr1" style="background-color:#DE7696;">
		<span class="wxts">温馨提示<span>
		<span class="scqr">×</span>
	  </div>
	  <div class="zz_nr2" style="line-height:146px;height:146px;display:block;font-size:18px;"></div>
	 
	  <div class="zz_nr3" >
			<a href="#wss" class="qrsc" style="background-color:#DE7696;">确认</a>
			<a href="#wss" class="tyjs">取消</a>	
	  </div>  
	</div>	

<div class="zz_nr_fbcg11" style="display:none;">
	  <div class="zdsc">
	  	提交成功，平台会核实处理
	  </div>
	   
	  <div class="zz_gbtk" >
			关闭
	  </div> 
	</div>


</div>
 <script type="text/javascript">
var change_color=$("#change_color").attr("value");
if(change_color==0){
	$(".jz_wdsc1").eq(0).addClass("jz_wdsc1_dj");
}
else if(change_color==1){
	$(".jz_wdsc1").eq(1).addClass("jz_wdsc1_dj");
}
else if(change_color==2){
	$(".jz_wdsc1").eq(2).addClass("jz_wdsc1_dj");
}
 $(".tyjs").click(function(){
	$(".zhezhao").css("display","none");
	$(".zz_nr_scdz").css("display","none");
});
$(".scqr").click(function(){
	$(".zhezhao").css("display","none");
	$(".zz_nr_scdz").css("display","none");
});
</script>
<script type="text/javascript" src="/Public/Home/js/hkhd1.js"></script>
<script type="text/javascript" src="/Public/Common/bootstrap/js/ajaxfileupload.js"></script>
<script type="text/javascript">
function cancel_collect(goods_id){
	 $(".zhezhao").css("display","block");
	$(".zz_nr_scdz").css("display","block");
  $(".zz_nr2").text("确认取消关注该选项吗？");
  window.goods_id=goods_id;
	
}

$(".qrsc").click(function(){
	$(".zz_nr_scdz").css("display","none");
	if($(".zz_nr2").text()=="确认取消收藏已选项吗？"){
			var k=0;
		var arra2=new Array();
		for(var j=0;j<$(".choose_all").length;j++){
			if($(".choose_all").eq(j).is(":checked")){
				arra2[k]=$(".choose_all").eq(j).attr("value");
				k++;
			}
		}
		$("#all_check_id").val(arra2);
		$.ajaxFileUpload({
			dataType: "json",
			url:"<?php echo U('JueZhi/delete_goods');?>",
	        type:'post',
	        data:{
	        	type:4,
	        	id:arra2
	        }, 
	        success:function(data){
	        	tyxs("取消关注成功");
	        }
	        
	
	    });
	}
	else if($(".zz_nr2").text()=="确认取消关注该选项吗？"){
		  $.ajaxFileUpload({
			dataType: "json",
			url:"<?php echo U('JueZhi/delete_goods');?>",
	        type:'post',
	        data:{
	        	type:3,
	        	id:goods_id
	        }, 
	        success:function(data){
	        	tyxs("取消关注成功");
	        }
	        
	
	    });
	}
})

function tyxs(message){
	     $(".zhezhao").css("display","block");
		 $(".zz_nr_fbcg11").css("display","block");
		 $(".zdsc").text(message);         
         
         djxs();
}
 function djxs(){
	 $(".zhezhao").click(function(){
		 $(".zhezhao").css("display","none");
  	     $(".zz_nr_fbcg11").css("display","none"); 
  	      location.reload();
	})
}
//将下拉标签显示
 $(".help_zl").parent().css("display",'none');
//左边浮动滑块距离头部
$(".hdmk").css("top",'50px');

//二设置左边点击背景
   $(".my_t5 li a").eq(4).css("backgroundColor","white").css("color","#e40050");
  $(".my_t5 li a").eq(4).siblings().css("backgroundColor","none");




//删除选中项目
$(".sc_button").click(function(){
	$(".zhezhao").css("display","block");
	$(".zz_nr_scdz").css("display","block");
	$(".zz_nr2").text("确认取消收藏已选项吗？");
	
})
//全选
$(".qx_button").click(function(){
	if($(this).is(':checked')){
		$(".choose_all").prop("checked", true);
		var arra1=new Array();
		for(var i=0;i<$(".choose_all").length;i++){
			arra1[i]=$(".choose_all").eq(i).attr("value");
		}
		$("#all_check_id").val(arra1);
	}
	else{
		$(".choose_all").prop("checked", false);
		$("#all_check_id").val(null);
	}
})
</script>

<?php echo W('Common/footer_esxz');?>
</body>
</html>